...
public class IfNode<T> extends Arity3Node<Boolean, T, T, T> {
    ...
    public IfNode(String name, int depth, Class<T> returnType) {
        super(name, depth, returnType, Boolean.class, returnType, returnType,
    ...
    public T evaluate() throws TypeMismatch { // Override to shortcut execution
        ...
        final Node<Boolean> test = this.arguments.get(0);
        final boolean testValue = test.evaluate();
        final int index = testValue ? 1 : 2;
        final Node<T> result = this.arguments.get(index);
